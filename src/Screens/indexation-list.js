...
// ExistingPatients
export default function IndexationList({ navigation }) {
            ...
            if (indexation != null) {
              if (indexation.length > 0) {
                return indexation.map((item, index) => {
                  return (
                    <IndexationCard key={index} data={item} onPress={() => navigation.replace('IndexationDetails' , {id : item.id})}/>
                  );
                });
              } else {
                return (
